...
public class HornetInHallownest() : Mod("HornetInHallownest"), ITogglableMod, ILocalSettings<HornetSaveData>,
    ...
    private void Bootstrap() {
        ...
        DebugServer.MapGet("/audio-test", req => AudioProbe.Test(req["repoint"], req["restore"])); // debug: live one-shot chain + mixer forensics (+ ?repoint=1, ?restore=1 experiments)
        DebugServer.MapGet("/mixer-census", _ => AudioProbe.Census()); // debug: loaded AudioMixer generations + bundle dup check, works at menu
        DebugServer.MapPost("/audio-probe", req => AudioProbe.SetArmed(req["on"])); // debug: log every one-shot call
        ...
        // before EnsureMounted: the AudioProbe hooks must see the very first addressables mounts
        AudioProbe.Install(); // DIAGNOSTIC: one-shot SFX gate tracer (armed via POST /audio-probe?on=true)
        ...
        moduleHost.Add(new GodhomeDifficultyBridgeModule());
