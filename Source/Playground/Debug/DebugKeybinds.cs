    ...
    private void TraceTick() {
        if (Input.GetKeyDown(KeyCode.F9)) {
            if (!tracing) {
                ...
                traceBuf = new List<string> { "t\tscene\ttransState\theroState\tonGround\tcReq\taccept\tvx\tvy\tx\ty" };
        ...
        traceBuf!.Add(string.Format(CultureInfo.InvariantCulture,
            "{0:F2}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7:F1}\t{8:F1}\t{9:F1}\t{10:F1}",
            Time.realtimeSinceStartup - traceT0, scene, hc.transitionState, hc.hero_state,
            hc.cState.onGround, hc.controlReqlinquished, hc.acceptingInput, v.x, v.y, p.x, p.y));
    }
}
