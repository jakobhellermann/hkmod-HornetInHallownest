...
public sealed class ValidationRunner {
    ...
    public ValidationRunner(ModuleHost host) {
        this.host = host;
    }
    ...
    public object List() {
        return new {
            scenarios = scenarios.Keys.OrderBy(k => k).ToArray(), modules = host.Modules.Select(m => m.Id).ToArray()
        };
    }
    ...
    public IEnumerator RunRoute(DevRequest req, Action<object?> respond) {
        ...
        try {
            foreach (var id in toDisable)
                if (host.Disable(id)) disabled.Add(id);
                else ctx.Fail($"could not disable module '{id}' (unknown or already off)");
